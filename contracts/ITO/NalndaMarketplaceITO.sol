...
contract NalndaMarketplaceITO is Context {
    ...
    constructor(address _NALNDA) {
        require(_NALNDA != address(0), "NalndaMarketplaceITO: NALNDA token's address can't be null!");
        NALNDA = IERC20(_NALNDA);
        master = INalndaMaster(_msgSender());
        lastOrderId = 0;
    }

    function listCover(INalndaITOBook _book, uint256 _tokenId, uint256 _price) external {
        require(Address.isContract(address(_book)) == true, "NalndaMarketplaceITO: Invalid book address!");
        require(_tokenId <= _book.coverIdCounter(), "NalndaMarketplaceITO: Invalid tokenId provided!");
        require(_book.ownerOf(_tokenId) == _msgSender(), "NalndaMarketplaceITO: Seller should own the NFT to list!");
        require(
            block.timestamp >= _book.secondarySalesTimestamp(),
            "NalndaMarketplaceITO: Listing for this book is disabled!"
        );
        require(
            block.timestamp >= _book.ownedAt(_tokenId) + master.secondarySaleAfterDays() * 1 days,
        ...
        emit ITOCoverListed(lastOrderId, _msgSender(), address(_book), _tokenId, _price);
    }

    function unlistCover(uint256 _orderId) external {
        require(_orderId <= lastOrderId, "NalndaMarketplaceITO: Invalid order id!");
        require(ORDER[_orderId].stage == Stage.LISTED, "NalndaMarketplaceITO: NFT not yet listed / already sold!");
        require(
            _msgSender() == ORDER[_orderId].seller || _msgSender() == master.owner(),
